...
const userdata = async() => {
    try {
        const response = await fetch(`${process.env.REACT_APP_MATCHED_PROFILE}?profileUrl=${profile.profileUrl}`, {
            method: 'POST',
...
const handleAccountDelete = async() => {
  try {
    const response = await fetch(process.env.REACT_APP_DELETE_REQUEST, {
      method: 'POST',
...
const handleAccountCancel = async() => {
  try {
    const response = await fetch(process.env.REACT_APP_CANCEL_REQUEST, {
      method: 'POST',
      body: JSON.stringify({username: user.username}),
      headers: {
        'Content-Type': 'application/json'
